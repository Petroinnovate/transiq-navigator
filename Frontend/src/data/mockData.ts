// ============================================================================
// DDR Intelligence Platform — Mock Data Layer (Offline / Demo Mode)
// Based on Rig 088TE / QTIF-790 / 10/02/2024
// Types re-exported from @/types/ddr.types for consistency
// ============================================================================

/**
 * Toggle demo mode globally. When true, API hooks can fall back to this data.
 * Set via VITE_DEMO_MODE env variable or hardcoded for development.
 */
export const DEMO_MODE = import.meta.env.VITE_DEMO_MODE === 'true' || false;

// ── Lightweight mock types (subset of full ddr.types) ───────────────────────
// These mirror the drillsight reference project's shapes for offline rendering.
// For full provenance types, import from @/types/ddr.types.

export interface MockKPIValue {
...
export interface MockRigSummary {
  rig_id: string;
  well_id: string;
  objective: string;
...
export const MOCK_RIGS: MockRigSummary[] = Array.from({ length: 267 }, (_, i) => {
  const rigNum = String(i + 1).padStart(3, '0');
  const suffix = ['TE', 'SA', 'BD', 'XR', 'MK'][i % 5];
  const status: MockRigSummary['status'] =
    i < 198
      ? i < 6 ? 'critical' : i < 29 ? 'warning' : 'normal'
      : i < 240 ? 'standby' : 'normal';
  return {
...
// ── Rig 088TE 24-Hour Timeline ──────────────────────────────────────────────

export const MOCK_RIG_088TE_TIMELINE: MockTimelineRow[] = [
...
export const MOCK_NPT_EVENTS: MockNPTEvent[] = [
...
export const MOCK_FLEET_TRENDS = {
...
export const MOCK_MUD_DATA_088TE = {
  current: {
...
// ── Well Design (Rig 088TE) ─────────────────────────────────────────────────

export const MOCK_WELL_DESIGN_088TE = [
