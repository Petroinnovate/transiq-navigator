...
const signupSchema = z.object({
    ...
    email: z.string().email('Please enter a valid email address'),
    password: z.string().min(8, 'Password must be at least 8 characters'), // backend requires min 8
...
const Auth = () => {
    ...
    const onLogin = async (data: LoginFormData) => {
        ...
        try {
            ...
            // FIX B: backend returns flat { access_token, user_id, email } — no nested user object
            if (!result?.access_token || !result?.user_id) {
                throw new Error('Invalid response format from server');
            }

            // Normalise into the user shape the rest of the app expects
            const user = {
    ...
    const onSignup = async (data: SignupFormData) => {
        ...
        try {
            ...
            // Build user object — include name from form since backend doesn't store it yet
            const user = {
                user_id: result.user_id,
                email: result.email,
                name: data.name,
            ...
            <div className="w-full max-w-md space-y-6">
                ...
                <Tabs defaultValue="login" className="w-full">
                    <TabsList className="grid w-full grid-cols-2 bg-slate-800 border-slate-700">
                        <TabsTrigger value="login" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-cyan-500 data-[state=active]:to-teal-500 data-[state=active]:text-white text-slate-300">
                            Login
                        </TabsTrigger>
                        <TabsTrigger value="signup" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-cyan-500 data-[state=active]:to-teal-500 data-[state=active]:text-white text-slate-300">
                            Sign Up
                        </TabsTrigger>
                    </TabsList>
