...
class RateLimiter:
    """In-memory rate limiter per API key / user identifier"""
...
class APIKeyMiddleware(BaseHTTPMiddleware):
    ...
    Authentication priority:
      ...
      2. Authorization: Bearer <jwt>  (React frontend flow)  ← FIX C
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        # Skip authentication for excluded prefixes (/auth/login, /auth/register, etc.)
        ...
        # ── FIX C: Accept X-API-Key OR Authorization: Bearer <jwt> ──────────

        # 1. Try X-API-Key header first (existing integrations)
        ...
        if not api_key:
            ...
            if auth_header.startswith("Bearer "):
                ...
                try:
                    ...
                    # Extend this mapping if you need per-user roles stored in the DB.
                    request.state.api_key = None
                    request.state.user_id = user_id
                    request.state.role = Role.OPERATOR

                    # Still apply RBAC check for JWT users
        ...
        if not api_key:
            logger.warning(
                f"Unauthorized: {request.client.host} — no X-API-Key or valid Bearer token"
            )
            return JSONResponse(
                ...
                content={
                    ...
                    "message": (
                        ...
                        "Include either 'X-API-Key' header or 'Authorization: Bearer <token>'."
                    ),
                },
            )

        # ── API key path (unchanged logic below) ─────────────────────────────

        if api_key not in valid_keys:
            logger.warning(
                f"Unauthorized: {request.client.host} — invalid API key: {api_key[:8]}…"
            )
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
                content={
        ...
        if not rate_limiter.is_allowed(api_key):
            logger.warning(
                f"Rate limit exceeded for API key: {api_key[:8]}… from {request.client.host}"
            )
            return JSONResponse(
                ...
                content={
                    ...
                    "message": (
                        f"Maximum {settings.RATE_LIMIT_PER_MINUTE} requests per minute exceeded. "
                        "Please slow down."
                    ),
                },
            )
        ...
        logger.debug(
            f"Authenticated: {request.client.host} key={api_key[:8]}… role={role.name}"
        )
