...
def test_p0_predictive():
    from app.kpi_engine.predictive_engine import forecast_kpi
...
# P1-1: DDR Parser
def test_p1_parser():
    from app.ddr.ddr_parser import parse_ddr_pdf, DDR_FIELD_PATTERNS, DDRParseResult
...
# P1-2: SPC Engine
def test_p1_spc():
    from app.ddr.spc_engine import compute_spc, compute_fleet_spc, SPCResult
...
def test_p1_citation():
    from app.ddr.citation_service import build_citation, save_extracted_metrics, get_metric_audit_trail
...
def test_p1_detector():
    from app.ddr.report_detector import detect_report_type, detect_report_type_from_text, DDR_KEYWORDS
...
def test_p2_fleet():
    from app.ddr.fleet_endpoints import router
    paths = [r.path for r in router.routes if hasattr(r, "path")]
...
def test_p2_rig():
    from app.ddr.rig_endpoints import router
    paths = [r.path for r in router.routes if hasattr(r, "path")]
...
def test_p2_audit():
    from app.ddr.audit_endpoints import router
    paths = [r.path for r in router.routes if hasattr(r, "path")]
...
def test_p2_ddr_core():
    from app.ddr.endpoints import router
    paths = [r.path for r in router.routes if hasattr(r, "path")]
...
def test_p2_rag():
    from app.ddr.drilling_rag_prompt import build_full_prompt, build_rag_context, DRILLING_RAG_SYSTEM_PROMPT
...
def test_p3_whatif():
    from app.kpi_engine.whatif_engine import KPI_RELATIONSHIPS, LEVER_RANGES, run_scenario
...
def test_p3_deduction():
    from app.processors.deduction import _split_text_into_chunks, _merge_facts, DeductionEngine
...
# P3-5: Semantic Dedup
def test_p3_dedup():
    from app.intelligence.validation import _title_similarity, _jaccard_similarity, validate_kpis
    # Identical titles
...
def test_p3_preindex():
    from app.embeddings.preindex import get_preindex_service, PreIndexService
...
def test_p3_trends():
    from app.ddr.trend_endpoints import router
    paths = [r.path for r in router.routes if hasattr(r, "path")]
...
def test_p3_widgets():
    from app.kpi_engine.widget_mapper import map_kpis_to_widgets
    kpis = [
...
def test_integration_financial_whatif():
    from app.kpi_engine.whatif_engine import run_scenario, FINANCIAL_MULTIPLIERS
